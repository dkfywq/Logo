...
def logo_eval(line, env, pre_operator_level=0):
    ...
    while not isinstance(line.current, list) and line.current in INFIX_SYMBOLS:
        ...
        if operator in INFIX_GROUPS[2]:
            ...
            if operator in INFIX_GROUPS[0]:
                ...
                result = logo_apply(proc, [result, logo_eval(line, env, 1)])
...
#         if not procedure:
#             error('I do not know how to {0}.'.format(token))
#         return apply_procedure(procedure, line, env)# def logo_eval(line, env):
...
def logo_apply(proc, args):
    ...
    if proc.isprimitive:
        ...
        for i in range(proc.arg_count):
            frame[variable_name(proc.formal_params[i])] = args[i]

        env.push_frame(frame)

        for each in proc.body:
            res = eval_line(Buffer(each),env)
...
class Environment(object):
    ...
    def lookup_variable(self, symbol):
        ...
        """
        "*** YOUR CODE HERE ***"
        for each in reversed(self._frames):
            if symbol in each:
                return each[symbol]
